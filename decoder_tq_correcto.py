...
# -*- coding: utf-8 -*-
"""
Decodificador TQ Correcto - Usa las mismas funciones que tq_server_rpg.py
"""
...
class TQCorrectDecoder:
    ...
    def parse_gps_datetime(self, fecha_gps: str, hora_gps: str) -> datetime:
        ...
        (Misma función que tq_server_rpg.py)
        """
        try:
    ...
    def load_positions_from_file(self) -> List[Dict]:
        """
        Carga y decodifica todas las posiciones del archivo usando las funciones del servidor
        """
        positions = []
                ...
                for line_num, line in enumerate(f, 1):
                    ...
                    if hex_match:
                        ...
                        if position:
                            ...
                            print(f"✅ Línea {line_num}: {position['device_id']} - {position['gps_timestamp'].strftime('%H:%M:%S')} - {position['latitude']:.6f}, {position['longitude']:.6f} - {position['speed_kmh']:.1f} km/h")
    ...
    def create_html_map(self, positions: List[Dict], output_file: str = 'mapa_tq_correcto.html') -> str:
        """
        Crea un mapa HTML con las posiciones decodificadas usando las funciones del servidor
        """
        if not positions:
...
<body>
    ...
    <div class="route-info">
        ...
        <small>Usando funciones de tq_server_rpg.py</small>
    </div>
    ...
    <script>
        ...
        print(f"✅ Mapa TQ correcto generado: {output_file}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Decodificador TQ Correcto')
    parser.add_argument('--archivo', '-f', default='RECORRIDO61674_011025.txt', 
                       help='Archivo de recorrido')
    parser.add_argument('--output', '-o', default='mapa_tq_correcto.html', 
                       help='Archivo de salida')
    
    args = parser.parse_args()
    
    print("=" * 70)
    print("🔧 DECODIFICADOR TQ CORRECTO")
    print("=" * 70)
    ...
    # Crear decodificador
    decoder = TQCorrectDecoder(args.archivo)
    ...
    if output_file:
        print()
        print("🎉 ¡Mapa TQ correcto generado exitosamente!")

...
# -*- coding: utf-8 -*-
"""
Generador de Mapas GPS Corregido - TQ Server RPG
Genera mapas HTML con ordenamiento correcto de datos GPS
"""
...
class FixedGPSMapGenerator:
    ...
    def parse_position_line(self, line: str) -> Optional[Dict]:
        ...
        try:
            ...
            if match:
                ...
                # PRIORIDAD: Usar fecha/hora GPS si está disponible, sino usar timestamp del log
                gps_timestamp = None
    ...
    def load_positions_from_log(self, device_id: str = None, date_filter: str = None) -> List[Dict]:
        """
        Carga posiciones desde el archivo de log con ordenamiento correcto
        """
        positions = []
        ...
        # ORDENAMIENTO CRÍTICO: Ordenar por timestamp GPS (no por timestamp del log)
        positions.sort(key=lambda x: x['timestamp'])
        
        print(f"📍 Cargadas {len(positions)} posiciones")
        if positions:
            print(f"   Rango GPS: {positions[0]['timestamp'].strftime('%H:%M:%S')} - {positions[-1]['timestamp'].strftime('%H:%M:%S')}")
            print(f"   Rango Log: {positions[0]['log_timestamp'].strftime('%H:%M:%S')} - {positions[-1]['log_timestamp'].strftime('%H:%M:%S')}")
    ...
    def create_html_map(self, positions: List[Dict], output_file: str = 'mapa_recorrido_corregido.html') -> str:
        """
        Crea un mapa HTML con ordenamiento correcto
        """
        if not positions:
...
<head>
    ...
    <title>Mapa GPS Corregido - Equipo {positions[0]['device_id']} - {positions[0]['timestamp'].strftime('%d/%m/%Y')}</title>
...
<body>
    ...
    <script>
        // Datos de posiciones (ordenados por timestamp GPS)
        ...
        // Crear línea del recorrido (ordenada por timestamp GPS)
        ...
        print(f"✅ Mapa HTML corregido generado: {output_file}")
        ...
        print(f"🗺️  Ordenamiento corregido por timestamp GPS")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generador de Mapas GPS Corregido - TQ Server RPG')
    ...
    parser.add_argument('--output', '-o', default='mapa_equipo_61674_corregido.html', 
                       help='Archivo de salida (default: mapa_equipo_61674_corregido.html)')
    ...
    print("=" * 70)
    print("🗺️  GENERADOR DE MAPAS GPS CORREGIDO - TQ SERVER RPG")
    print("=" * 70)
    ...
    # Crear generador
    generator = FixedGPSMapGenerator(args.log)
    ...
    if output_file:
        print()
        print("🎉 ¡Mapa corregido generado exitosamente!")
        print(f"📂 Abrir: {os.path.abspath(output_file)}")
        print("💡 El mapa usa ordenamiento por timestamp GPS (no por timestamp del log)")
        print("🗺️  Las líneas ahora deberían seguir un recorrido coherente")
